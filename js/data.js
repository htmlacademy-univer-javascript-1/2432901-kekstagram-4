...
const createMessage = () => {
  const sentenceNumber = getRandomInteger(COMMENT_MIN_COUNT, COMMENT_MAX_COUNT);
  const message = [];
...
const createComment = (_, id) => (
  {
    id,
    avatar: `img/avatar-${getRandomInteger(1, AVATAR_COUNT).toString()}.svg`,
...
const getPictures = () => Array.from({length: PICTURE_COUNT}, getPhotoData);
